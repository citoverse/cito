...
Using 'cito', you can fit neural networks with the dnn function. Here you can specify all 

While this training is done on the CPU, all models can be trained on the GPU with device = "cuda". This is suggested if you are working with large models and networks. 
...
You can plot the networks structure to give you a visual feedback of the created object. Be aware that this may take some time for large networks.  
```{r plotnn, eval = FALSE}
plot(nn.fit)
```
...
By default  all layers are fitted with ReLu as activation layer.
$$
relu(x) = max (0,x)
$$
...
In order to see where your model might start overfitting the addition of a validation set can be useful. With dnn() you can put validation = 0.x and the defined percentage will be not used for training and only for validation after each epoch. The loss plot will show you how the two losses behave. 
...
We can choose the model with minimal validation loss. By setting nn.fit$use_model_epoch to the model with the lowest validation loss, this model will be used for new predictions. 
```{r epoch1,eval = FALSE}
...
## Training parameters  
Training a neural network can be difficult. You probably have to try out some variations of hyperparameters and explore which work best for your data set.

### Regularization 
#### Elastic net regularization
If elastic net is used, 'cito' will produce a sparse, generalized neural network. The L1/L2 loss can be  controlled by the arguments alpha and lambda.  The elastic net loss is defined as following.

$$
 loss = \lambda * [ (1 - \alpha) * |weights| + \alpha |weights|^2 ]
$$

If a single alpha value is provided each layer will get regularized the same. However, you can regularize each layer individually by providing a vector of alpha values the same length as there are hidden layers + 1, the +1 comes from the input layer. With NA you can turn off generalization for specific layers.
...
Dropout can also regularize your network. During training in every epoch defined percentage of nodes gets left out.
...
### Learning rate
The lr parameter controls the so called learning rate. This parameter defines the step size the optimizer takes. If set too high the optimizer might not find a good optimum because you shoot across certain valleys. If set too low the training process takes too long to be viable. The default value of 0.01 usually performs quite good. 
