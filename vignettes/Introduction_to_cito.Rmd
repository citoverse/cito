...
## Setup 
Before using cito you have to make sure that torch is installed and is running so first you need to run the following chunk of code
```{r setuptorch}
...
```{r optim,eval = FALSE}
# adam optimizer with learning rate 0.002 with a slightly changed betas to 0.9, 0.999 and eps to 1.5e-08
...
nn.fit <- dnn(Species~., data = data,lr = 0.01, lr_scheduler= "step", config = list(lr_scheduler.step_size = 10, lr_scheduler.gamma = 0.5))


```

### Early Stopping 
Adding early stopping criteria helps saving time by stopping the training process at the current epoch if the validation loss of the current epoch is biggger than the validation loss n epochs early. The n can be defined by the ealry_stopping argument. It is required to set validation > 0. 
...
## Continuing Training 
For the case that the specified number of epochs were not enough, cito provides the continue_training() function. It allows to continue the training process were dnn() quitted. 
...
```{r continue_training,eval = T,fig.show='hide',out.lines = 3}
# simple example, simply adding another 32 epochs to the training process   
...
```{r continue_training2,eval = T,fig.show='hide', out.lines = 3}

# picking a specific net and training from there on with a smaller learning rate and a smaller batchsize
nn.fit <- continue_training(nn.fit, continue_from = 10, epochs = 32, changed_params = list(lr = 0.001, batchsize = 16))
