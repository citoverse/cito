...
#'
#' Helpd you create custom optimizer for \coode{\link{dnn}}. It is recommended to set learning rate in \coode{\link{dnn}}.
#'
#' @param optim character string defining the type of optimizer
...
config_optimizer<- function(optim = c("adam", "adadelta", "adagrad", "rmsprop", "rprop", "sgd"), ... ){

  optim <- match.arg(tolower(optim), choices =  c("adam", "adadelta", "adagrad", "rmsprop", "rprop", "sgd", "lbfgs"))
  out <- list()
  out$optim <- optim
...
config_lr_scheduler<- function(scheulder =  c("lambda", "multiplicative", "one_cycle", "step"),...){
