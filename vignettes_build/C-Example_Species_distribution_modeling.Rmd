...
abstract: "This vignette shows working examples of how to fit (multi-) species distribution models with cito"
date: "`r Sys.Date()`"
output:
 rmarkdown::html_vignette:
    toc: true
vignette: >
  %\VignetteIndexEntry{Example SDM}
...
Training neural networks is tricky compared to other ML algorithms that converge more easily (due to various reasons). The purpose of this vignette is to provide an example workflow and to point out common caveats when training a neural network
...
At the end of the training, the losses start to get jumpy, which can be a sign of potential overfitting. We can control that by using a weak regularization (but we only want a L2 regularization, so we set alpha to 1.0):
...
Species react differently to cvTemp (different niches).
